...
RSpec.describe(SeasonStats) do
  before(:each) do
    ...
    @warehouse = DataWarehouse.new(CSV.read(locations[:games],     headers: true,     header_converters: :symbol), CSV.read(locations[:teams],     headers: true,     header_converters: :symbol), CSV.read(locations[:game_teams],     headers: true,     header_converters: :symbol))
    data = @warehouse.data_by_season("20122013")
    id_team_key = @warehouse.id_team_key
    @season_stats = SeasonStats.new(data, id_team_key)
  end

  context("Season Stats tests") do
